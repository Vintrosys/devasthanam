erpnext.PointOfSale.ItemCart = class {
	...
	make_cart_totals_section() {
		...
		this.$totals_section.append(
			...
			<div class="archanai-btn">${__("Archanai")}</div>
	...
	bind_events() {
		...
		// custom
		this.$component.on("click", ".archanai-btn", async function () {
			if ($(this).attr("style").indexOf("--blue-500") == -1) return;

			await me.events.archanai();
		});
		// end
	...
	toggle_checkout_btn(show_checkout) {
		if (show_checkout) {
			...
			this.$totals_section.find(".archanai-btn").css("display", "flex"); // custom
	...
	highlight_checkout_btn(toggle) {
		if (toggle) {
			...
			// custom
			this.$cart_container.find(".archanai-btn").css({
				"background-color": "var(--blue-500)",
			});
			// end
		} else {
	...
	load_invoice() {
		...
		if (frm.doc.docstatus === 1) {
			this.$totals_section.find(".checkout-btn").css("display", "none");
			this.$totals_section.find(".archanai-btn").css("display", "none"); // custom
			this.$totals_section.find(".edit-cart-btn").css("display", "none");
		} else {
			this.$totals_section.find(".checkout-btn").css("display", "flex");
			this.$totals_section.find(".archanai-btn").css("display", "flex"); // custom
