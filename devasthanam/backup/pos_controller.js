erpnext.PointOfSale.Controller = class {
	...
	init_item_cart() {
		this.cart = new erpnext.PointOfSale.ItemCart({
			...
			events: {
				...
				archanai: () => this.open_archanai(), // custom
	...
	open_archanai() {
		...
		var data = []
		const old_entries = me.frm.doc.custom_archanai_list || [];
		if(old_entries){
			old_entries.forEach(row => {
				data.push({
		...
		const dialog = new frappe.ui.Dialog({
			title: 'Enter Archanai Details',
			size: "large",
			fields: [
				{
					label: 'Archanai List',
					fieldname: 'archanai_list',
					fieldtype: 'Table',
					cannot_add_rows: 0,
					in_place_edit: true,
					data: data,
					fields: [
						...
						{
							fieldtype: 'Link',
							fieldname: 'natchathiram',
							label: 'Natchathiram',
							options: 'Natchathiram',
							in_list_view: 1,
							reqd: 1
						}
					]
			...
			primary_action_label: 'Add All to Table',
			primary_action(values) {
				const entries = values.archanai_list || [];
				
				me.frm.doc.custom_archanai_list = []
				entries.forEach(row => {
					me.frm.add_child('custom_archanai_list', {
						member_name: row.member_name,
						natchathiram: row.natchathiram
					});
				})
